...
router.post('/message', async (req, res) => {
	...
	// Obtener mensajes de la base de datos
		var msj_inicial = await controlador.texto.cargar_msj_incial();
		var msj_cliente = await controlador.texto.cargar_mensajes();
		var msj_no_cliente = await controlador.texto.cargar_no_cliente();
		var msj_aut_exitosa = await controlador.texto.cargar_aut_exitoso();
		var msj_aut_erroneo = await controlador.texto.cargar_aut_errroneo();
		var msj_preguntas_rut = await controlador.texto.cargar_preguntas_rut();
		var msj_preguntas_EPA = await controlador.texto.cargar_preguntas_EPA();
		var msj_fin_EPA = await controlador.texto.cargar_fin_EPA();
		var msj_ya_registrado = await controlador.texto.cargar_ya_registrado();
	...
	var local_function = {
		...
		ya_registrado : async function()
		{
			console.log("[local_function] :: [ya_registrado]");

			resultado.action = msj_ya_registrado.action;
			resultado.messages.push(msj_ya_registrado.messages[0]);
			resultado.additionalInfo.authValidity = false;
		},
					...
					if(context !== '' && typeof context !== "undefined") 
					{
						if(mensaje !== '' && typeof mensaje !== "undefined") 
						{
							...
							if((context.lastMessageFrom == "NOTIFICATION" && diff < 24) || (localStorage.getItem("NOTIFICATION"+user.id) == "NOTIFICATION" && diff < 24))
							{
								...
								{
				                	if(num_intentos_EPA == null) { num_intentos_EPA = 0; }
				                ...
				                if(localStorage.getItem("bot_bci_"+conversationID) !== null)
				                {
									...
									if(pregunta_rut[0] == "rut" && pregunta_rut[1] == "true" )
									{
										...
										if(bandera_vali_rut)
										{
											...
											var get_persona = await controlador.valid_clie.get_persona(mensaje_rut,user.id)
											...
											if(get_persona === true)
											{
												...
												{                         
													...
													if(respuesta_rut == null) { localStorage.setItem("respuesta_rut"+conversationID, mensaje); }
											...
											else if(get_persona === "YA_REGISTRADO")
											{
												await local_function.ya_registrado();

												await local_function.remove_localStorage();
											}
											else /*if(get_persona === "No_Existe")*/
											{
												await local_function.no_cliente();
										...
										if(bandera_vali_serie)
										{
											...
											if(axios_CEDU.status)
											{
												...
												else
												{
													...
													if(num_intentos == null) { num_intentos = 0; }
